...
if(!PRODUCTION){
	if(!empty($username) && !empty($hospital_name) && !empty($_REQUEST["password"]) && !empty($_REQUEST["mobile"])){
		if(strlen($username)>=4 && strlen($hospital_name)>=4){
			if(strlen($_REQUEST["password"])>=6 && strlen($_REQUEST["password"])<=12 && preg_match("#[0-9]+#", $_REQUEST["password"]) && preg_match("#[a-zA-Z]+#", $_REQUEST["password"])){
				...
				$output='{"status":"failure", "remark":"Password must be 6-12 characters long, at least one number and at least one letter"}';
			}
		}else{
